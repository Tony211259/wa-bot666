    ...
    lolhuman.on('chat-update', async(lol) => {
        try {
            ...
            switch (command) {
                case 'help':
                    var punya_wa = "0@s.whatsapp.net"
                    var ini_text = "*Verifed by WhatsApp*"
                ...
                case 'ytmp3':
                    ...
                    ini_txt += `* Tunggu Sebentar, Musik segera dikirim*\n`
                ...
                case 'jadwaltv':
                    ...
                    get_result = get_result.result
                    ini_txt = `*INFO TV ${channel.toUpperCase()}*\n\n Streaming? disini https://tony211259.github.io/test/hd/tv/${channel}.html\n\n\n *Jadwal TV ${channel.toUpperCase()}*\n\n`
                    for (var x in get_result) {
                        ini_txt += `${x} - ${get_result[x]}\n`
                    }
                    reply(ini_txt)
                ...
                case 'tebakumur':
                    ...
                    ini_name = args.join(" ")
                    if (args.length == 0) return reply(`Example: ${prefix + command} LoL Human`)
                    get_result = await fetchJson(`http://api.lolhuman.xyz/api/tebakumur?apikey=erdwpehub28&name=${ini_name}`)
                    get_result = get_result.result
                    ...
                    // Entertainment
                case 'darkjoke':
                    get_result = await fetchJson(`https://api.zeks.xyz/api/darkjokes?apikey=apivinz`)
                    ini_buffer = await getBuffer(get_result.result)
                    lolhuman.sendMessage(from, ini_buffer, image, { quoted: lol })
                    break
                case 'wancak':
                    ...
                    break
                case 'estetik':
                    ini_buffer = await getBuffer(`https://api.zeks.xyz/api/estetikpic?apikey=apivinz`)
                    lolhuman.sendMessage(from, ini_buffer, image, { quoted: lol })
                    break
                ...
                case 'ssweb':
                    ...
                    ini_link = args[0]
                    ini_buffer = await getBuffer(`https://docs-jojo.herokuapp.com/api/ssweb?url=${ini_link}&device=tablet`)
                    lolhuman.sendMessage(from, ini_buffer, image, { quoted: lol })
                    break
                case 'ssweb2':
                    if (args.length == 0) return reply(`Example: ${prefix + command} http://api.lolhuman.xyz`)
                ...
                if (args.length == 0) return reply(`Example: ${prefix + command} LoL Human`)
                ini_txt = args.join(" ")
                ini_buffer = await getBuffer(`https://fzn-gaz.herokuapp.com/nulis?text=${ini_txt}`)
                lolhuman.sendMessage(from, ini_buffer, image, { quoted: lol })
                    break
                case 'pornhub':
                case 'glitch':
